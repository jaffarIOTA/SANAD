/**
 * The service, driven over HTTP.
 *
 * `test/contract/origination-api.test.ts` asserts what the *document* says.
 * This asserts that the running service does it — over a real socket, with
 * real headers, through the same pipeline a partner would hit.
 *
 * The emphasis is on the controls rather than on the happy path: a rate
 * refused, a tenant not claimable, a replay not re-executing, one partner
 * unable to see another's business.
 */
...
const AGGREGATOR: PartnerPrincipal = {
...
const OTHER_PARTNER: PartnerPrincipal = {
...
const READ_ONLY: PartnerPrincipal = {
...
const registry: CredentialRegistry = {
  findByTokenDigest(digest) {
    const table = new Map<string, PartnerPrincipal>([
...
// -- The happy path, and what it does and does not mean -----------------------
...
describe('raising a request', () => {
  it('accepts a well-formed request and parks it with the servicing platform', async () => {
    ...
    // Partner channels consult the servicing platform first. A 201 is not
    // appetite; it is "well formed, now with the institution".
...
describe('SH-01 — a rate cannot be posted', () => {
  it('refuses a proportion-shaped field rather than dropping it', async () => {
    ...
    // And the offending field name is not echoed back.
...
describe('§8 — the client cannot choose its tenant, channel or identity', () => {
  ...
  it('refuses an aggregator that omits the merchant’s mandate', async () => {
    ...
    // The aggregator credential means the domain demands a mandate, and the
    // body did not carry one. An aggregator may introduce a merchant; it may
...
describe('§8 — idempotency', () => {
  ...
  it('replays the original result rather than executing twice', async () => {
    ...
    // The decisive assertion: the same request, not a second one.
  ...
  it('does not create a second request on replay', async () => {
    ...
    const matching = listed.body.items.filter(
      (i: { partnerReference?: string }) => i.partnerReference === 'replay-probe',
    );
  ...
  /**
   * The security property, stated as itself.
   *
   * Scoping idempotency by tenant alone is subtly wrong: several partners
   * operate inside one tenant, so partner B presenting partner A's key would
   * receive A's stored response — carrying A's counterparty, trade and
   * amounts. This asserts the data never crosses, which is stronger than
   * asserting the replay header is absent.
   */
...
describe('authentication and scope', () => {
  ...
  it('reports another partner’s request as absent, not forbidden', async () => {
    ...
    // 404, not 403: the API does not confirm the existence of other
    // partners' business.
...
describe('withdrawal', () => {
  ...
  it('is idempotent by nature, which is why it is a PUT', async () => {
    ...
    // A different key, so this is not an idempotency-store replay — the
...
describe('transport', () => {
  ...
  it('refuses malformed JSON without a stack trace', async () => {
    ...
    const body = (await raw.json()) as { reason: string };
  ...
  it('reads no gateway-injected header', async () => {
    // A forged consumer header must change nothing: the service authenticates
    // independently of whatever ran in front of it (§5).
