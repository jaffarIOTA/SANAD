 ...
 * The decision engine.
 *
 * The properties under test are the four that SDD §6.6 calls non-negotiable:
 * determinism, a full trace, bilingual reasons derived from that trace, and a
 * challenger that is measured and never applied.
 *
 * The tenant comparisons matter as much as the individual assertions. The same
 * applicant, put to two institutions' policies, gets two different answers —
 * from the same engine, with no branch on which institution it is.
 */
...
function policyFor(tenant: TenantCode): CreditPolicy {
  const versions = expectOk(loadCreditPolicyVersions(tenant, 'wasl-distributor'));
  return expectOk(resolveEffectivePolicy(versions, 1_791_000_000n));
}

function decide(tenant: TenantCode, overrides: SnapshotOverrides = {}) {
...
describe('determinism (BR-C02)', () => {
  ...
  it('records the snapshot capture time rather than reading a clock', () => {
    ...
    expect(decision.snapshotCapturedAtEpochSeconds).toBe(1_791_000_000n);
  });
});
...
describe('a strong applicant is approved, and the two institutions size it differently', () => {
  it('approves at the bank, sized on trade with the anchor and capped by invoice size', () => {
    ...
    // 3x median monthly trade is 180,000,000, capped to 4x the largest single
    // invoice at 160,000,000.
    expect(decision.assignedLimitMinorUnits).toBe(160_000_000n);
  ...
  it('approves at the fintech, sized on the lower of trade and bank inflow', () => {
    ...
    expect(decision.outcome).toBe('APPROVE');
    expect(decision.grade).toBe('A');
    expect(decision.assignedLimitMinorUnits).toBe(120_000_000n);
  });
...
describe('knockouts', () => {
  it('declines an inactive registration at both institutions', () => {
    for (const tenant of ['bank-a', 'fintech-b'] as const) {
...
describe('data sufficiency', () => {
  ...
  it('does not require a consent the institution does not use', () => {
    // The fintech's policy never reads the bureau, so it never asks for that consent.
  ...
  it('refers on stale data rather than deciding on figures nobody stands behind', () => {
    const decision = decide('bank-a', { tradeHistory: { retrievedSecondsAgo: 999_999 } });
    expect(decision.outcome).toBe('REFER');
...
describe('the score, the grade and the reasons', () => {
  it('refers a middling applicant with the characteristics it fell short on', () => {
    const decision = decide('bank-a', {
      tradeHistory: {
        largestBuyerSharePerTenThousand: 9000,
        withAnchor: {
          medianMonthlyValueMinorUnits: 20_000_000n,
    ...
    // Derived from the trace, worst shortfall first, and capped at three so the
    // counterparty gets an explanation rather than a wall of text.
  ...
  it('picks the highest grade band the score reaches', () => {
    const grades = [
      { grade: 'A', minScore: 85, outcome: 'APPROVE' as const },
      { grade: 'B', minScore: 70, outcome: 'APPROVE' as const },
      { grade: 'D', minScore: 0, outcome: 'DECLINE' as const },
    ];
...
describe('limit sizing and caps', () => {
  it('nets the limit down by exposure the institution already carries', () => {
    const decision = decide('bank-a', {
      exposure: { coreBankingExposureMinorUnits: 60_000_000n },
    });
    expect(decision.assignedLimitMinorUnits).toBe(100_000_000n);
  ...
  it('never exceeds the programme’s remaining headroom', () => {
    const decision = decide('bank-a', {
      programme: {
        programmeLimitMinorUnits: 1_000_000_000n,
        programmeUtilisedMinorUnits: 990_000_000n,
      },
    });
    expect(decision.assignedLimitMinorUnits).toBeLessThanOrEqual(10_000_000n);
  });

  it('caps a single counterparty’s share of the programme', () => {
    const decision = decide('bank-a', {
      programme: {
        programmeLimitMinorUnits: 200_000_000n,
        programmeUtilisedMinorUnits: 0n,
      },
    });
    // 15% of 200,000,000 is 30,000,000.
    expect(decision.assignedLimitMinorUnits).toBe(30_000_000n);
  ...
  it('rounds the limit down, never up', () => {
    const decision = decide('bank-a', {
      tradeHistory: {
        withAnchor: {
          medianMonthlyValueMinorUnits: 50_033_333n,
          largestSingleInvoiceMinorUnits: 900_000_000n,
        },
      },
    });
    // 3x 50,033,333 is 150,099,999, rounded down to the nearest 100,000.
    expect(decision.assignedLimitMinorUnits).toBe(150_000_000n);
  });

  it('refers rather than approving a limit too small to be worth issuing', () => {
    const decision = decide('bank-a', {
      exposure: { coreBankingExposureMinorUnits: 159_000_000n },
    });
    expect(decision.outcome).toBe('REFER');
    expect(decision.assignedLimitMinorUnits).toBe(0n);
  });
...
describe('champion and challenger', () => {
  it('records the challenger’s decision and applies the champion’s', () => {
    ...
    // The challenger measured more generously. It still did not apply.
...
describe('policy loading refuses what it cannot stand behind', () => {
  it('refuses a policy section the engine does not recognise', () => {
    const policy = policyFor('bank-a') as unknown as Record<string, unknown>;
...
describe('effective dating', () => {
  ...
  const v2: CreditPolicy = { ...base, version: '2.0.0', effectiveFromEpochSeconds: '1800000000' };

  it('resolves the version in force at the moment being evaluated, not the newest', () => {
    const before = expectOk(resolveEffectivePolicy([base, v2], 1_791_000_000n));
    expect(before.version).toBe('1.0.0');

    const after = expectOk(resolveEffectivePolicy([base, v2], 1_800_000_001n));
  ...
  it('refuses when two versions share an effective date', () => {
    const clash: CreditPolicy = { ...base, version: '1.0.1' };
    const result = resolveEffectivePolicy([base, clash], 1_791_000_000n);
    expect(result.ok).toBe(false);
    if (result.ok) return;
